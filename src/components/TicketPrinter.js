// TicketPrinter.js
// Este módulo contiene la lógica de generación e impresión de tickets.
...
const generateFullOrderPrintContent = (table, order) => {
  ...
  const formattedDate = formatDate(order[0].fecha_pedido);
    ...
    <html>
      ...
      <body>
        <div>
          ...
          ${table.nota ? `<p style="font-weight: bold;">Nota: ${table.nota}</p>` : ''}
          <div class="dashed-line"></div>
          <p style="font-weight: bold;">Total: $${order.reduce((acc, p) => acc + (Number(p.cantidad) * Number(p.precio)), 0)}</p>
          <div class="dashed-line"></div>
